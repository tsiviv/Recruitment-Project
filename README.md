...
7. [מגבלות ידועות ועבודה עתידית](#מגבלות-ידועות-ועבודה-עתידית)
...
## מגבלות ידועות ועבודה עתידית

- **`DatabaseService` הוא Stub במכוון** — אין חיבור DB אמיתי בשלב זה (כפי שהוגדר בהיקף המשימה). כל הפעולות (שמירת פנייה, בדיקת סקר קיים וכו') מוגדרות אך אינן שומרות דבר בפועל. השאילתה המלאה לדוח החודשי (SQL) קיימת ומתועדת, אך אינה מחוברת בפועל למסד נתונים.
- **אין Authentication/Authorization** — כל ה-Endpoints ציבוריים; ברגע שתתווסף הזדהות יש לחדש את בדיקת ה-Antiforgery (ראו סעיף אבטחה).
- **תיקיית קבצים מקומית כברירת מחדל** — כאשר לא מוגדר `ServerIdentity`/קונפיגורציית סביבה, קבצים נשמרים לתיקיית `Uploads/` מקומית בתוך תיקיית הפרויקט (ולא לנתיב רשת ייעודי) — מתאים לפיתוח מקומי בלבד.
